...
for s1 = 1:1
    for s2 = 1:1
        ...
        for i = 1 : length(tau)
            ...
            AvgR1(i) = mean(R1_Sim);
            AvgR2(i) = mean(R2_Sim);
            SeR1(i) = std(R1_Sim)/sqrt(20);
            SeR2(i) = std(R2_Sim)/sqrt(20);
            
            Avg_SL(i) = d1*AvgR1(i) + d2*AvgR2(i);
            VarSL = d1^2*var(R1_Sim) + d2^2*var(R2_Sim);
            Se_SL(i) = sqrt(VarSL);

            EvalR1(i) = CalcR1(tau(i),0.0625);
            EvalR2(i) = CalcR2(tau(i),0.0625);
            Eval_SL(i) = d1*AvgR1(i) + d2*AvgR2(i);
        ...
        saveas(gcf,['Visitor2000_SecurityLevel_S1_' num2str(s1) '_S2_' num2str(s2) '.png'],'png')
        saveas(gcf,['Visitor2000_SecurityLevel_S1_' num2str(s1) '_S2_' num2str(s2) '.fig'],'fig')
