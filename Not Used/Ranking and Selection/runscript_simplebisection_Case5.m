...
log_test_name='Debug_log_SimpleBisection_Case5_RandomInitial.txt';
log_test=fopen(log_test_name, 'w'); 
problemname      = 'AirportSecurity';
problemparam     = [0 2 4 2 200 0.001 0.001*sqrt(m0*geomc)];
...
x0               = [0 0; ceil((Budget-Beta1)/c1) ceil(Budget/c2)]; 
                             % [min; max]
...
solvername       = 'cgRSPLINE';
solverparam      = [1 20 1000 3.5 0.4 8 10 .002 0.1 1 m0 geomc ...
                    2000000 1.1 0.95 0.65]; 
...
solverseedparam  = [1, 0];   % nsolvseeds = 1
                             ...
                             % requires actual seeds values
solverseed       = 20000;    %48906,10000,20000
logfilename      = 'cgRSPLINE-on-AirportSecurity_SimpleBisection_Case5_RandomInitial';  
                             % log file base name 
budget           = 40000000;    % 'numfinalsols'=1

                                                         
%% CALL cgR-SPLINE
solverhandle=str2func(solvername);
[ncalls, x, Fn, FnVar, Grad, GradVar, problemseed, solverseed, other]...
    = solverhandle(x0, problemname, problemparam, problemseed, ...
    problemseedparam, solverparam, solverseed, solverseedparam, budget,...
    logfilename);