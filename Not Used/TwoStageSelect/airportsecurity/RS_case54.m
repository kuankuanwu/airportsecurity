...
if sum(sum(snima >= 0)) ~= numel(snima)
  return
end
...
for j=1:big_s1
 ...
 [tau,AvgWaitingTime,Se_AvgWaitingTime,MinTau_Budget,flag2,mobs,iseed,...
    ...
    warmup,mk*second_test(cur_best_s),...
    1e-3,Tol);
...
disp(['R&S Case54 result:  BestS = (' 'trial:' num2str(t) ' end, best_s=' num2str(cur_best_s) ])

xlswrite('R&S_Case54_best_s.xls',trial_best_s);
xlswrite('R&S_Case54_best_tau.xls',trial_best_tau);
xlswrite('R&S_Case54_best_SE.xls',trial_best_SE);
xlswrite('R&S_Case54_best_test.xls',trial_best_test);
xlswrite('R&S_Case54_TotalCost.xls',trial_cost);
xlswrite(strcat('R&S_Case54_average_tau_Trial',int2str(t),'.xls'),sencond_tauavma);
xlswrite(strcat('R&S_Case54_SE_Trial',int2str(t),'.xls'),sencond_sema);
xlswrite(strcat('R&S_Case54_test_Trial',int2str(t),'.xls'),second_test);
end
