...
%                   given S1 = 0 or S2 = 0.
    ...
    if S1 == 0
        ...
        if AvgWaitingTime*60 < Epsilon
            tau = 0;
            flag2 = 0;
            fprintf(log_test,'\t\tEnd Solving Tau: ((S1,S2), tau, WT, SE_WT, ncalls) = ((%d, %d),%f,%f,%f, %d)\n',S1, S2, tau, AvgWaitingTime*60-Epsilon, Se_AvgWaitingTime, mobs);
            disp(['End Solving Tau: ((S1,S2), tau, WT, SE_WT, ncalls) = ' num2str(S1) ',' num2str(S2) ',' num2str(tau) ',' num2str(AvgWaitingTime*60-Epsilon) ','  num2str(Se_AvgWaitingTime) ',' num2str(mobs)])
            ...
            Constraints = [AvgWaitingTime*60-Epsilon Cost-Budget];
        else
            tau = [];
            flag2 = 1;
            fprintf(log_test,'\t\tEnd Solving Tau: INFEASIBLE\n');
    ...
    %if S1 <= size(RefMatrix_tau,1) && S2 <= size(RefMatrix_tau,2)
        %MinTau_Budget = GetMinTau(S1,S2);
             ...
             %disp(['End Solving Tau(skip): ((S1,S2), tau, WT, SE_WT, ncalls) = ' num2str(S1) ',' num2str(S2) ',' num2str(tau) ',' num2str(AvgWaitingTime*60-Epsilon) ','  num2str(Se_AvgWaitingTime) ',' num2str(mobs)])
