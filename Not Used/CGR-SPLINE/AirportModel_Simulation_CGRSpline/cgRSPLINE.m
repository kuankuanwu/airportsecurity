...
while 1
    ...
    while 1
		...
		if StartificationFlag == 0
            ...
            if LinearFalg ==0
               ...
               [x0,solvseed]=GetInput_Linear(solvseed,r,xmin,xmax,Acurr); 
            end  
        else
    ...
    % calculate mk 
    if sparam(10)==1
        mk=ceil(sparam(11)*sparam(12)^(k-1));
    else
...
if r > 2 
  ...
  if   Acurr(r-1,1) == Acurr(r-2,1) && Acurr(r-1,2) == Acurr(r-2,2)  
     [u, seed] = mrg32k3a(seed);
     x0(1) = floor(xmin(1) + u*(xmax(1) - xmin(1)) + 0.5);
     Upper = floor((Budget - Beta1 - c1*x0(1))/c2);
     Lower = floor(max((Budget - Beta2 - c1*x0(1))/c2,0));
  ...
  td = (Acurr(r-1,2)-Acurr(r-2,2))/ (Acurr(r-1,1)-Acurr(r-2,1)); 
  
  [u, seed] = mrg32k3a(seed);
  x0(1) = floor(xmin(1) + u*(xmax(1) - xmin(1)) + 0.5);
 
  
  x0(2)= floor(Acurr(r-1,2)+ td*(x0(1)-Acurr(r-1,1))+0.5);
...
%             NConstraintCov, Nphat, alphar, logfid);
    ...
    %disp(['x1ConstraintCov =' num2str(x1ConstraintCov)])
    sparam(5)=(log(sqrt(x1ConstraintCov*mk))-log(epsilon))/log(mk); %ERROR_BG
    disp(['sparam(5) =' num2str(sparam(5))])
    delta=sparam(5);
...
while (1)
    ...
    disp(['Flag2=' num2str(flag2)])
    disp(['Flag1=' num2str(flag1)])
    %ncalls = ncalls + mk;
    if AirportSecurityFlag == 1
    ...
    ctr=ctr+1;
    disp(stack1)
    disp(['nrow-ctr=' num2str(nrow-ctr)])
    if nrow-ctr <1   % stack is empty
        ...
        disp(['stackflag=' num2str(stackflag)])
        return
    end
    if nrow-ctr > 1
