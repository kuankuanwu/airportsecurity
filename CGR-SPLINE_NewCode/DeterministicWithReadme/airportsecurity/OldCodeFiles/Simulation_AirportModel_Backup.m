...
function iseed = arrive(tau,iseed,Num_Warmup)
    ...
    if RiskValue <= tau
        iseed = arrive_non_selectee(iseed,Num_Warmup);
    else
        iseed = arrive_selectee(iseed,Num_Warmup);
    end
    
end

function iseed = arrive_non_selectee(iseed,Num_Warmup)
    global mu1
    ...
    if IdleServerIdx == 0
        ...
        if Num_EndedWaitingVisitors >= Num_Warmup
            ...
            update_sum_variables(DelayTime)
            update_sum_variables_NonSelecteeLane(DelayTime)
        end
        ...
        [ProcessTime,iseed(2 + IdleServerIdx)] = rexpo(1/mu1,iseed(2 + IdleServerIdx));
        %ProcessTime = 1/mu1;
        NextDepartureTime1(IdleServerIdx) = Sim_Time + ProcessTime;
    end
end

function iseed = arrive_selectee(iseed,Num_Warmup)
    global mu2
    ...
    if QueueNum_SelecteeLane == 0
        ...
        if IdleServerIdx == 0
            ...
            if Num_EndedWaitingVisitors >= Num_Warmup
                ...
                update_sum_variables(DelayTime)
                update_sum_variables_SelecteeLane(DelayTime)
            end
            Num_EndedWaitingVisitors = Num_EndedWaitingVisitors + 1;
            Num_EndedWaitingVisitors_SelecteeLane = Num_EndedWaitingVisitors_SelecteeLane + 1;
            
            ServerStatus2(IdleServerIdx) = 1;
            [ProcessTime,iseed(end - length(ServerStatus2) + IdleServerIdx)] = rexpo(1/mu2,iseed(end - length(ServerStatus2) + IdleServerIdx));
            %ProcessTime = 1/mu2;
...
function iseed = depart_non_selectee(iseed,Next_Departure_ServerIdx,Num_Warmup)
    ...
    if QueueNum_NonSelecteeLane == 0
        ...
        [ProcessTime,iseed(2+Next_Departure_ServerIdx)] = rexpo(1/mu1,iseed(2+Next_Departure_ServerIdx));
        %ProcessTime = 1/mu1;
...
function iseed = depart_selectee(iseed,Next_Departure_ServerIdx,Num_Warmup)
    ...
    if QueueNum_SelecteeLane == 0
        ...
        [ProcessTime,iseed(end - length(ServerStatus2) + Next_Departure_ServerIdx)] = rexpo(1/mu2,iseed(end - length(ServerStatus2) + Next_Departure_ServerIdx));
        %ProcessTime = 1/mu2;
...
function update_sum_variables(WaitingTime)
...
function update_sum_variables_NonSelecteeLane(WaitingTime)
...
function update_sum_variables_SelecteeLane(WaitingTime)
